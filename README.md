...
##### Có gì mới trong phiên bản 0.6.0
...
Phiên bản 0.6.0 sửa kết quả `result_only` để chỉ trả task được hỏi, giữ checkpoint
...
Kết quả gọn giờ lấy tin nhắn kết luận cuối của worker thay vì phần log lệnh ở
đầu JSONL.
Task có quyền ghi giờ được MAC tạo hoặc xác minh worktree trước khi gọi worker.
MAC từ chối chạy trên `main`/`master`, không cho hạ isolation đã lưu và công bố
rõ worktree cùng branch thực tế trong kết quả dispatch.
...
Version 0.6.0 also adds free-tier API routing for Google Gemini and Cloudflare
...
Version 0.6.0 makes `result_only` truly task-scoped, preserves explicit
...
Compact results now extract the worker's final assistant message rather than
leading command logs from provider JSONL.
Workspace-write tasks now get a verified persistent worktree before provider
dispatch. MAC rejects protected branches and isolation downgrades, then reports
the effective worktree and branch to the Master.
