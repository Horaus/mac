...
def validate_profile(profile: ExecutionProfile) -> None:
    ...
    if profile.context.history_limit < 0 or (profile.context.max_input_tokens is not None and profile.context.max_input_tokens <= 0): raise ValueError("invalid context budget")
...
def profile_from(value: dict[str, Any] | None) -> ExecutionProfile:
    ...
    profile = ExecutionProfile(value.get("model"), value.get("profile"), value.get("reasoning_effort"), value.get("sandbox"), ContextPolicy(int(context.get("history_limit", 6)), context.get("knowledge_mode", "summary"), context.get("max_input_tokens", 12000), tuple(context.get("allowed_paths", ()))))
