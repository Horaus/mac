...
def _menu(project="."):
    ...
    def guide():
        ...
        if lang == "vi":
            topics = [("Gọi MAC", ["Sau khi cấu hình MCP, dán một câu này vào khung chat:", "Kết nối tới MAC MCP server đã cài đặt trong project này.", "MAC tự nạp rule/knowledge; Master sẽ hỏi tiếp mục tiêu, số worker và kết quả cần đạt."]),
            ...
            topics = [("Call MAC", ["After configuring MCP, paste this one line into the chat:", "Connect to the MAC MCP server installed in this project.", "MAC loads its rules/knowledge; the Master then asks for the goal, worker count and expected result."]),
