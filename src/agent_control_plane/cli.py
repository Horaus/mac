...
def _menu(project="."):
    ...
    def guide():
        ...
        if lang == "vi":
            topics = [("Gọi MAC", ["Sau khi cấu hình MCP, dán một câu này vào khung chat:", "Kết nối tới MAC đã cài đặt từ Horaus/mac qua MCP.", "MAC tự nạp rule/knowledge; Master sẽ hỏi tiếp mục tiêu, số worker và kết quả cần đạt."]),
            ...
            topics = [("Call MAC", ["After configuring MCP, paste this one line into the chat:", "Connect to MAC installed from Horaus/mac through MCP.", "MAC loads its rules/knowledge; the Master then asks for the goal, worker count and expected result."]),
