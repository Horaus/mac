...
def _menu(project="."):
    ...
    def guide():
        ...
        if lang == "vi":
            topics = [("Gọi MAC", ["Sau khi kết nối MCP, dán câu này vào khung chat:", "Gọi MAC (Multi-Agent Control) qua MCP và làm việc theo toàn bộ hướng dẫn MAC cung cấp.", "Khi giao việc, nói rõ mục tiêu, số worker mong muốn và kết quả cần đạt."]),
            ...
            topics = [("Call MAC", ["After connecting MCP, paste this into the chat:", "Call MAC (Multi-Agent Control) through MCP and follow all instructions provided by MAC.", "When assigning work, state the goal, desired worker count, and required outcome."]),
